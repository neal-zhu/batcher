...
const batcherABI = [
  "function execute(uint, uint, address, bytes) payable",
]
...
task("xen_claim_rank", "claimRank for xen").setAction(async (taskArgs, hre) => {
  ...
  let tx = await batcher.execute(0, 100, taskArgs.xen, xen.interface.encodeFunctionData("claimRank", [1]));
  await tx.wait();

  console.log("XEN balance: ", (await xen.balanceOf(signer.address)).toString());

}).addParam("batcher", "address of batcher contract").addParam("xen", "address of xen contract");

task("xen_claim_reward", "claim reward for xen").setAction(async (taskArgs, hre) => {
  const signer = (await hre.ethers.getSigners())[0];

  const xen = new hre.ethers.Contract(taskArgs.xen, xenABI, signer);

  // increase and mine time so we can claimReward
  await increaseTime(24*60*60);

  const batcher = new hre.ethers.Contract(taskArgs.batcher, batcherABI, signer);
  tx = await batcher.execute(0, 100, xen.address, xen.interface.encodeFunctionData("claimMintRewardAndShare", [signer.address, 100]));
  await tx.wait();
  ...
  networks: {
    hardhat: {
      forking: {
        ...
        url: `https://polygon-mainnet.g.alchemy.com/v2/U0SRzRQYhqlHexksOsp8PoRelu7odDgB`,
        accounts: {
